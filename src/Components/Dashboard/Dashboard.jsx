...
const Dashboard = () => {
  ...
  const [selectedTruck, setSelectedTruck] = useState(null); // State for selected truck for details view

  useEffect(() => {
    // Simulating incoming truck data
    const truckData = [
      {
        id: 1,
        number: "TN-01-A-1234",
      ...
      // Add more truck data with different destinations if needed
    ];

    // Simulating outgoing truck data
    const outgoingTruckData = [
      {
        id: 8,
        number: "KL-14-H-5555",
  ...
  const handleFilterTrucks = () => {
    // Assuming trucks data would have weight and destination properties in real cases
    const filtered = trucks.filter((truck) => {
      return (
        (!driverForm.weight || truck.weight <= driverForm.weight) &&
        (!driverForm.destination ||
          truck.destination === driverForm.destination)
      );
    });
    setFilteredTrucks(filtered);
    setMessage("Route is allocated to driver Shreyas Deshmukh"); // Set the message when filtering
  };

  const handleTruckDetails = (truckId) => {
    const truck = [...trucks, ...outgoingTrucks].find(
  ...
  return (
    <div className="dashboard-container">
      <button
        ...
        onClick={() => navigate("/dashboard/consignment")}
      >
        Register New Driver
      </button>
      <button
      ...
      <h1 className="title">
        PostTrucker <span className="dashboard-highlight">Dashboard</span>
      </h1>
      <div className="dashboard-grid">
        {/* Left Side: Trucks Section */}
        <div className="section trucks-section">
              ...
              {trucks.map((truck) => (
                <div
                  ...
                  className={`dashboard-card ${
                    truck.status === "Delayed" ? "truck-issue" : ""
                  } ${truck.sos ? "truck-sos" : ""}`}
                >
                  <p className="truck-number">Truck Number: {truck.number}</p>
                  ...
                  {truck.sos && <p className="sos-alert">SOS Alert!</p>}
                  <button onClick={() => handleTruckDetails(truck.id)}>
                    Manage
                  </button>
                </div>
              ))}
        ...
        <div className="section driver-form-section">
          ...
          <div className="dashboard-card driver-form">
            ...
            <input
              ...
              value={driverForm.weight}
              onChange={(e) =>
                setDriverForm({ ...driverForm, weight: e.target.value })
              }
              placeholder="Max Weight"
            />
            <label>Destination: </label>
            <select
              value={driverForm.destination}
              onChange={(e) =>
                setDriverForm({ ...driverForm, destination: e.target.value })
              }
            >
              <option value="">Select Destination</option>
              <option value="City A">Rudraram Post Office</option>
              <option value="City B">Kukatpally Post Office</option>
              <option value="City C">Bachupally Post Office</option>
      ...
      {/* Display truck details modal (conditionally) */}
      {selectedTruck && (
        <div className="truck-details-modal">
          <h3>Truck Details</h3>
          <p>Truck Number: {selectedTruck.number}</p>
          <p>
            Arrival/Departure Time:{" "}
            {selectedTruck.arrival || selectedTruck.departure}
          </p>
          <p>Status: {selectedTruck.status}</p>
          <p>Destination: {selectedTruck.destination}</p>
          <p>Weight: {selectedTruck.weight}</p>
          {selectedTruck.sos && <p className="sos-alert">SOS Alert!</p>}
